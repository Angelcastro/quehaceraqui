 ...
 */
package es.angelcastro.quehaceraqui;

import es.angelcastro.quehaceraqui.bd.queaceraquiDBManagerMySQL;
...
public class Main extends HttpServlet {
    ...
    private static final Logger logger = Logger.getLogger(queaceraquiDBManagerMySQL.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        queaceraquiDBManagerMySQL.connect(dbServer, dbName, dbUser, dbPassword);

        //Comprueba si se ha podido realizar la conexión
        if (queaceraquiDBManagerMySQL.isConnected()) {
            ...
            if (action != null && !action.isEmpty()) {
                ...
                if (action.equals(ACTION_EDIT_REQUEST)) {
                    ...
                    Rutas rutas = queaceraquiDBManagerMySQL.getPersonByID(id);
                    ...
                    redirectTo("rutas_detail.jsp", request, response);
                    ...
                    updatePersonWithRequestData(rutas, request);
                    //Se actualiza en la BD la persona
                    queaceraquiDBManagerMySQL.updatePerson(rutas);
                    ...
                    Rutas rutas = new Rutas();
                    updatePersonWithRequestData(rutas, request);
                    queaceraquiDBManagerMySQL.insertPerson(rutas);
                    response.sendRedirect("Main?action=");
                    ...
                    queaceraquiDBManagerMySQL.deletePersonById(id);
                    response.sendRedirect("Main?action=");
                    ...
                    ArrayList<Rutas> rutasList = queaceraquiDBManagerMySQL.getPersonsList();
                ...
                ArrayList<Rutas> personsList = queaceraquiDBManagerMySQL.getPersonsList();
                request.setAttribute("rutasList", personsList);
                redirectTo("rutas_list.jsp", request, response);
            }
     ...
     * @param request
     */
    private void updatePersonWithRequestData(Rutas rutas, HttpServletRequest request) {
        rutas.setNombre(request.getParameter("nombre"));
        rutas.setAlias(request.getParameter("alias"));
        rutas.setKilometros(request.getParameter("kilometros"));
        rutas.setDificultad(request.getParameter("dificultad"));
        rutas.setLatitud(request.getParameter("latitud"));
        rutas.setLongitud(request.getParameter("longitud"));
        rutas.setLocalidad(request.getParameter("localidad"));
        rutas.setProvincia(request.getParameter("provincia"));
        rutas.setPais(request.getParameter("pais"));
        rutas.setComentario(request.getParameter("comentario"));
        ...
        if(request.getParameter("deletePhoto")!=null && request.getParameter("deletePhoto").equals("on")) {
            ...
            if(!rutas.getFotoruta().isEmpty() && rutas.getFotoruta()!=null) {
                deletePhotoFile(request, rutas.getFotoruta());
                rutas.setFotoruta("");
            }
        } else { //No se ha pedido dejar sin foto
            //Guardar la imagen en el servidor y asignar el nombre a la persona
            String fotoruta = savePhotoFile(request);
            if(fotoruta!=null) {
                if(rutas.getFotoruta().isEmpty() || rutas.getFotoruta()==null) {
                    //Si no tenía foto, le asigna la nueva
                    rutas.setFotoruta(fotoruta);
                } else {
    ...
    private String savePhotoFile(HttpServletRequest request) {
        ...
        try {
            ...
            Part filePart = request.getPart("fotoruta");
